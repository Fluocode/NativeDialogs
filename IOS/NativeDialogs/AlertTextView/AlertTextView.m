...
- (void)drawRect:(CGRect)rect
{
    ...
    CGRect rec = self.accessibilityFrame, rec2 = CGRectMake(12.0, 5.0, 260.0, 25.0);
    CGFloat height = 0;
...
-(id)initWithTitle:(NSString *)title message:(NSString *)message delegate:(id)delegate cancelButtonTitle:(NSString *)cancelButtonTitle otherButtonTitles:(NSString *)otherButtonTitles, ...{
    if (self = [self initWithTitle:title message:message delegate:delegate cancelButtonTitle:cancelButtonTitle otherButtonTitles:otherButtonTitles, nil])
    {
        if(title!=nil && message!=nil){
            self.titleField = [self.view.subviews objectAtIndex:0];
            self.messageField = [self.view.subviews objectAtIndex:1];
            
        }else if (title!=nil) {
            self.titleField = [self.view.subviews objectAtIndex:0];
        }else if(message!=nil){
            self.messageField = [self.view.subviews objectAtIndex:0];
        }        
        [self.view setNeedsDisplay];
    }
    return self;
