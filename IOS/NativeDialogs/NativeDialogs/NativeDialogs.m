 ...
 * NativeDialogs
 *
 * Based in the code by Mateusz Mackowiak on 02.02.2013.
 * Created by Emmanuel Gomez on 23.07.2020
 * Copyright (c) 2020 Fluocode. All rights reserved.
 */
...
#define UTILITIES_CONTEXT "NativeUtilitiesContext"
...
FREObject isDarkMode(FREContext ctx, void* functionData, uint32_t argc, FREObject argv[] ){
    NativeDialogControler *nativeDialogController = functionData;
    FREObject returnVal;
    FRENewObjectFromBool([nativeDialogController isDarkMode], &returnVal);
    return returnVal;
}

FREObject getStatusBarHeight(FREContext ctx, void* functionData, uint32_t argc, FREObject argv[] ){
    NativeDialogControler *nativeDialogController = functionData;
    FREObject returnVal;
    FRENewObjectFromDouble([nativeDialogController getStatusBarHeight], &returnVal);
    return returnVal;
}

FREObject statusBarStyleLight(FREContext ctx, void* functionData, uint32_t argc, FREObject argv[] ){
    NativeDialogControler *nativeDialogController = functionData;
    uint32_t lightStyle;
    FREGetObjectAsBool(argv[0], &lightStyle);
    [nativeDialogController statusBarStyleLight:lightStyle];
    return NULL;
}
...
void NativeDialogsContextInitializer(void* extData, const uint8_t* ctxType, FREContext ctx, uint32_t* numFunctionsToTest, const FRENamedFunction** functionsToSet)
{
    ...
    {
        numOfFun = 6;
    }else if(strcmp((const char *)ctxType, UTILITIES_CONTEXT)==0)
    {
        numOfFun = 3;
    }
    ...
    *numFunctionsToTest = numOfFun;
    ...
    if(strcmp((const char *)ctxType, LIST_DIALOG_CONTEXT)==0){
        ...
        func[0].functionData = nativeDialogController;
        func[0].function = &getStatusBarHeight;
        
        func[1].name = (const uint8_t *) "isDarkMode";
        func[1].functionData = nativeDialogController;
        func[1].function = &isDarkMode;
        
        func[2].name = (const uint8_t*) "statusBarStyleLight";
        func[2].functionData = nativeDialogController;
        func[2].function = &statusBarStyleLight;
        
       // func[3].name = (const uint8_t*) "statusbarColor";
       // func[3].functionData = nativeDialogController;
       // func[3].function = &statusbarColor;
    }
