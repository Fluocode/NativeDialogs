...
package com.fluocode.nativeANE.dialogs
{
	...
	public class NativeProgressDialog extends AbstractNativeDialog
	{
		...
		public function show(cancleble:Boolean=false,indeterminate:Object=null):Boolean
		{
			...
			try{
				if(isAndroid()){
					trace("showProgressPopup",_progress,_secondary,_style,_title,_message,_maxProgress,cancleble,_indeterminate,_androidTheme);
					_isShowing = true;
					return true;
				}
				else if(isIOS()){
					trace("showProgressPopup",_progress/_maxProgress,null,_style,_title,_message,cancleble,_indeterminate,_iosTheme);
					_isShowing = true;
					return true;
				}
				return false;
		...
		public function showProgressbar(cancleble:Boolean=false):Boolean
		{
			...
			try{
				if(isAndroid()){
					trace("showProgressPopup",_progress,_secondary,_style,_title,_message,_maxProgress,cancleble,false,_androidTheme);
					_isShowing = true;
					return true;
				}
				else if(isIOS()){
					trace("showProgressPopup",_progress,null,_style,_title,_message,cancleble,false,_iosTheme);
					_isShowing = true;
					return true;
				}
				return false;
		...
		public function showSpinner(cancleble:Boolean=false):Boolean
		{
			try{
				...
				if(isAndroid()){
					...
					trace("showProgressPopup",_progress,null,_style,_title,_message,cancleble,true,_iosTheme);
					_isShowing = true;
					return true;
				}
				return false;
				...
				try{
					if(_isShowing){
						if(isAndroid()){
							...
							trace("updateProgress",value/_maxProgress);
						}
						_progress = value;
						return true;
					}else{
