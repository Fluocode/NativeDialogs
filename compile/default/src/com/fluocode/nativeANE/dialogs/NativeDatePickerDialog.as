...
package com.fluocode.nativeANE.dialogs
{
	...
	public class NativeDatePickerDialog extends AbstractNativeDialog
	{
		 ...
		 * com.fluocode.nativeANE.events.NativeDialogEvent.CLOSED
		 ...
		 * 
		 * @see com.fluocode.nativeANE.dialogs.NativeDatePickerDialog.defaultTheme
		 * @see com.fluocode.nativeANE.dialogs.NativeDatePickerDialog.show
		 * 
		 * @throws Error - if the call was unsuccessful. Or will dispatch an Error Event.ERROR if there is a listener.
		 * 
		 * @event com.fluocode.nativeANE.dialogs.NativeDialogEvent.OPENED
		 * com.fluocode.nativeANE.dialogs.NativeDialogEvent.CLOSEED
		 * com.fluocode.nativeANE.dialogs.NativeDialogEvent.CANCELED
		 */
		public function show(cancelable:Boolean = true) : Boolean
		{
			try{
				...
				{
					if(isAndroid()){
						trace("show", _title,_message,dateToString(_date),_buttons,_displayMode, _is24HourView,_cancelable, _theme, _minDate.time, _maxDate.time);
					}else if(isIOS()){
						trace("show", _title,_message,dateToIOSTimestamp(_date),_buttons,_displayMode, _is24HourView,_cancelable, _theme, dateToIOSTimestamp(_minDate), dateToIOSTimestamp(_maxDate));
					}
				}
				else
				{
					if(isAndroid()){
						trace("show", _title,_message,dateToString(_date),_buttons,_displayMode, _is24HourView,_cancelable, _theme);
					}else if(isIOS()){
						trace("show", _title,_message,dateToIOSTimestamp(_date),_buttons,_displayMode, _is24HourView,_cancelable, _theme);
					}
				}
		...
		public function setDate(value:Date):Boolean
		{
			...
			try{
				if(_isShowing){
					if(isAndroid()){
						trace("setDate",dateToString(value));
					}else if(isIOS()){
						trace("setDate",dateToIOSTimestamp(value));
					}
				}
				_date = value;
				return true;
		 ...
		 * @default com.fluocode.nativeANE.dialogs.NativeDatePickerDialog#DEFAULT_THEME
		 */
